...
type PublicMaintenanceSpec struct {
	...
	// Title is the maintenance page title (max 255 characters).
	// +kubebuilder:validation:MaxLength=255
	// +optional
	Title string `json:"title,omitempty"`

	// Message is the maintenance page message (max 2000 characters).
	// +kubebuilder:validation:MaxLength=2000
	// +optional
	Message string `json:"message,omitempty"`

	// EstimatedTime is the estimated maintenance duration (max 100 characters).
...
type PublicHealthCheckSpec struct {
	...
	// Mode is the health check mode (default: http).
	// +optional
	Mode string `json:"mode,omitempty"`

	// Interval is the seconds between health checks (default: 30).
	// +kubebuilder:validation:Minimum=1
	// +optional
	Interval int `json:"interval,omitempty"`

	// UnhealthyInterval is the seconds between checks when unhealthy (default: 30).
	// +kubebuilder:validation:Minimum=1
	// +optional
...
type PublicTargetSpec struct {
	...
	// InternalPort is an internal port mapping on the target.
	// +kubebuilder:validation:Minimum=1
	// +kubebuilder:validation:Maximum=65535
	// +optional
	...
	// PathMatch is the path matching type.
	...
	// RewriteMatch is the rewrite match type.
	// +kubebuilder:validation:Enum=exact;prefix;regex;stripPrefix
	// +optional
	RewriteMatch string `json:"rewriteMatch,omitempty"`

	// Priority is the load-balancing priority (1–1000, default 100).
	// +kubebuilder:validation:Minimum=1
	// +kubebuilder:validation:Maximum=1000
	// +optional
	Priority int `json:"priority,omitempty"`
...
// PublicResourceSpec defines the desired state of a Pangolin public-resource.
// Each CR produces one entry in the blueprint's public-resources block.
type PublicResourceSpec struct {
	...
	// Ssl enables SSL on the Pangolin resource.
	// +kubebuilder:default=true
	// +optional
	...
	// Maintenance configures the maintenance page (Enterprise Edition only).
	// +optional
...
// +kubebuilder:printcolumn:name="Domain",type="string",JSONPath=".spec.fullDomain"
// +kubebuilder:printcolumn:name="Age",type="date",JSONPath=".metadata.creationTimestamp"

// PublicResource is the Schema for Pangolin public-resources.
// Each instance produces one entry in the blueprint's public-resources block.
